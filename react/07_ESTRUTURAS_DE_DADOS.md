...
No React, as estruturas de dados mais comuns que você gerenciará no estado são **Arrays** e **Objetos**. Embora o JavaScript não tenha um framework de coleções tão extenso quanto o do Java, seus `Array` e `Object` nativos são incrivelmente versáteis.
...
Use a sintaxe de espalhamento (`...`) para criar um novo array com o novo item.
...
function adicionarTarefa() {
  ...
  setTarefas([...tarefas, 'Dominar o State']);
}
```
...
Use a sintaxe de espalhamento (`...`) para criar um novo objeto, copiando as propriedades antigas e, em seguida, sobrescrevendo a que você deseja alterar.
...
Ao lidar com objetos aninhados, você precisa usar o espalhamento em cada nível do objeto que está atualizando.
  ...
  setPerfil({
    ...
    detalhes: {
      ...
      endereco: {
        ...
        cidade: 'Londres' // 4. Sobrescreva o valor final
      }
    }
  });
}
...
## **Escolhendo a Estrutura de Estado Certa**
...
- **Evite redundância:** Não armazene dados no estado que podem ser calculados a partir de props ou outras variáveis de estado durante a renderização.

---

## **Resumo das Operações Imutáveis**

| Tarefa | Java `ArrayList` (Mutável) | Estado do React (Imutável) |
| --- | --- | --- |
| **Adicionar** | `lista.add(novoItem);` | `setLista([...lista, novoItem]);` |
| **Remover** | `lista.removeIf(item -> ...);` | `setLista(lista.filter(item -> ...));` |
| **Atualizar** | `lista.get(i).setCampo(val);` | `setLista(lista.map(item -> ... ? novoItem : item));`|
| **Atualizar Campo**| `obj.setCampo(val);` | `setObj({...obj, campo: val});` |

---

## **Resumo**

> No React, gerencie coleções com **Arrays** do JavaScript e dados estruturados com **Objetos**. Sempre trate o estado como **imutável**. Use métodos que retornam novos arrays (`map`, `filter`) e a sintaxe de espalhamento (`...`) para criar cópias atualizadas do seu estado, o que garante que o React renderize corretamente.
