# 🔁 Estruturas de Repetição em React (Renderização de Listas)

Em React, você não usa laços tradicionais como `for` ou `while` diretamente no seu JSX. Em vez disso, você renderiza listas de componentes transformando um array de dados em um array de elementos JSX. O método padrão para isso é o `Array.prototype.map()` embutido do JavaScript.

---

## **O Método `.map()`**

O método `.map()` cria um **novo array** chamando uma função em cada elemento do array original. Em React, nós o usamos para mapear nossos dados (ex: um array de objetos) para um array de componentes.
...
// Então, renderiza o novo array dentro de um <ul>
...
4.  **Último Recurso: Índice:** Usar o índice do array (`(item, indice) => ... key={indice}`) **não é recomendado** se a lista puder ser reordenada, ter itens adicionados ou filtrada. Isso pode levar a bugs com o estado do componente e atualizações incorretas do DOM. Use-o apenas para listas estáticas e imutáveis.
...
function UsuariosAtivos({ usuarios }) {
  ...
  // Então, mapeie o array filtrado para elementos JSX
  return (
    <ul>
...
-   Para **"parar"** (parar a renderização mais cedo), você pode usar o método `.slice(0, indice)` no array antes de mapear.
...
> Para renderizar uma lista de itens em React, use o método **`.map()`** no seu array de dados para transformá-lo em um array de elementos JSX. Sempre forneça uma prop **`key` estável e única** ao elemento de nível superior dentro do map para garantir atualizações eficientes e sem bugs. Para a renderização condicional de itens de lista, use **`.filter()`** antes de mapear.
