# 🔧 **Entrada (Formulários) e Saída (Exibindo Estado) de Dados**
...
Em React, um "componente controlado" é um elemento de formulário de entrada cujo valor é controlado pelo estado do React.

1.  Nós criamos uma parte do estado usando `useState` para armazenar o valor da entrada.
...
Vamos construir um formulário que pergunta o nome de um usuário e o exibe ao vivo.
...
-   `useState('')`: Nós inicializamos o estado `nome` com uma string vazia.
...
-   `setNome(...)`: Nós atualizamos o estado, o que faz com que o componente seja renderizado novamente, exibindo o novo nome tanto no `<h2>` quanto no próprio `<input>`.
...
function FormularioDeInscricao() {
  ...
  const handleSubmit = (event) => {
    // Impede o comportamento padrão do navegador de recarregar a página inteira
...
### **Pontos Chave para o Envio**

-   `onSubmit={handleSubmit}`: O manipulador é colocado na tag `<form>`, não no botão.
-   `event.preventDefault()`: Isso é **crucial**. Sem ele, o navegador tentará enviar o formulário e recarregar a página, o que não é como as Aplicações de Página Única (SPAs) como o React funcionam.
-   Os dados já estão em nosso estado (`email`), então podemos usá-los diretamente no manipulador de envio (ex: enviá-los para uma API, atualizar o estado do pai, etc.).
...
| **Sinal de "Concluído"**| O programa prossegue ou termina | Manipulador de eventos `onSubmit` no `<form>`. |

---

## **Resumo**

> Em React, use **componentes controlados** para lidar com a entrada do usuário. Armazene o valor das entradas do formulário em **`useState`**, atualize-o com **`onChange`**, e processe os dados finais no manipulador **`onSubmit`** do `<form>` (lembrando sempre de chamar `event.preventDefault()`).
