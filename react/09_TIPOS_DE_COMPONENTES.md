...
No React, existem duas maneiras principais de definir um componente: como uma **função JavaScript** (Componente Funcional) ou como uma **classe ES6** (Componente de Classe).

Embora você encontre ambos em bases de código existentes, os **Componentes Funcionais com Hooks são o padrão moderno e recomendado para todo novo desenvolvimento.**

---

## **1. Componentes Funcionais (O Padrão Moderno)**

Um componente funcional é uma função JavaScript simples que aceita `props` como argumento e retorna JSX. Para adicionar estado ou recursos de ciclo de vida, você usa **Hooks** (como `useState` e `useEffect`).
...
function Contador({ contagemInicial }) {
  ...
  // 2. Efeitos colaterais (como buscar dados ou definir timers) são gerenciados com useEffect
  useEffect(() => {
...
-   **Analogia**: Isto é como uma classe Java tradicional. Tem um construtor, variáveis de instância (`this.state`) e métodos (`this.setState`).
...
| --- | --- | --- |
| **Sintaxe** | Função simples | Estende `React.Component` |
...
3.  **Reutilização de Lógica Mais Fácil:** Os Hooks (como `useEffect` e Hooks personalizados) fornecem uma maneira muito mais limpa e poderosa de compartilhar lógica com estado entre componentes em comparação com padrões mais antigos como Mixins ou Componentes de Ordem Superior (HOCs) usados com classes.
4.  **Melhor para Otimização:** Componentes funcionais podem ser mais fáceis para a equipe do React otimizar no futuro.

> **Conclusão**: Embora você deva entender como ler um Componente de Classe, você deve sempre escolher **Componentes Funcionais com Hooks** para qualquer novo código React que escrever.
